...
if [ -f "$WALLPAPER_DIR/$SELECTED_WALLPAPER.jpg" ]; then
  ...
  #  sed -i -e "s|monitor = .*|monitor = $MONITOR|" "$TARGET_FILE"
    sed -i -e "s|path = .*|path = \$HOME/Pictures/Wallpapers/$SELECTED_WALLPAPER.jpg|" "$TARGET_FILE"
    sed -i -e "s|path = .*|path = \$HOME/Pictures/Wallpapers/$SELECTED_WALLPAPER.jpg|" "$TARGET_FILE2"
